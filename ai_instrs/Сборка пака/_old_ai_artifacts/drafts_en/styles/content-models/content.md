...
- In the `<Sections>` block, place inputs, algorithm, instructions, or outputs according to @draft(content-sections).
...
- **Implementation Instructions:** contains concrete steps, rules, and action sections formatted according to @draft(content-sections).
...
3. Format the execution part: apply sections from @draft(content-sections). In the structured model, place them under the `### Implementation Instructions` heading.
